...
/*
  ...
  As it turns out, defining new\delete and their [] variants in any translation unit overrides the usual new\delete
  operators. This allows us to track any and all allocations made during execution of the tests. As all memory should
  have been released when the test function exits, we easily see if there are any leaks by checking if delete was not
  called to new'ed pointers
*/
